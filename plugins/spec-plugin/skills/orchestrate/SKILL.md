...
3. **PREP — write the session scaffolding and commit it before any engineer spawns** (engineers read these from the spec workspace branch, so they must exist there first):
   - **setup-playbook** (`specs/<version>/setup-playbook.md`) — spawn a recon step (an `engineer` is fine) to inspect `code_repo` and document exactly how to spin up a code worktree for *this* repo: branch base, how to copy `.env`/gitignored files, dependency install, how to run gates/tests, known toolchain gotchas. **Record the EXACT copy-paste gate commands** — the precise `cd <path> && <test/typecheck/lint>` invocations WITH any required env prefixes (e.g. a runtime-version env var) and cwd handling — so engineers don't rediscover invocation (shell state doesn't persist between Bash calls; past runs burned turns on "cwd was reset" and trial-and-error typecheck invocation). **Seed it from the previous version's playbook if one exists** (a diff, not a rewrite). **Confirm completeness with the user** before execution.
...
2. **Spawn a fresh engineer for the story.** One engineer per story: it builds the story, hands off to QA, reports back, and is torn down. Run up to the chosen parallelism concurrently (a new engineer per in-flight story).
