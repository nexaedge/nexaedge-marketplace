...
You answer one question precisely and cheaply: **does this symbol exist, what is its real signature, and where is it defined?** (Or, if it doesn't: what's the nearest real thing?) This is the antidote to code that assumes `partner.slug` or `CreditOffer.find_by_offer` and turns out to be wrong.
